...
const Card = ({data, openReportPopup}) => {
    ...
    const date = getDate(data.updatedOn || data.updatedAt)
    const time = getTimeDate(data.updatedOn || data.updatedAt, "HH-MM")[1]
            ...
            <div>
                <div className="user-info">
                    ...
                    <div className="content-card-report">
                        <div className="icon-button small" onClick={(e)=>openReportPopup(e, type, data._id)}>Report</div>
                    </div>
                </div>
                <div className="body">
                ...
                <div className="card-bottom">
                    ...
                    <div className="card-bottom-right">
                        <button>share</button>
                        {type === "note" && (<Link to={`/user/personal-folder/note/${data._id}`}>View</Link>)}
                        {type === "file" && data.fileType === "pdf" && (<Link to={`/user/personal-folder/pdf/${data._id}`}>View</Link>)}
                        {type === "file" && (<a href={thumbnail} download={data.name}>Download</a>)}
                    </div>
                </div>
            </div>
        </div>
