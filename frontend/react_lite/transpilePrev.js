...
function jsxToCreateElementMine4Util(jsx){
    ...
    const isExpression = jsx.match(/({|<)/)[1] == "{"
    ...
    if(isExpression){
        ...
        if(!expression.includes(" ")){
            return [
                [
                    {type: "TEXT_ELEMENT", props: {}, children: proceedingText}, 
                    {type: "EXPRESSION", props: {}, children: `<noquote>${expression}<noquote>`}
                ]
                , next]
        }else{
            return [
                [
                    {type: "TEXT_ELEMENT", props: {}, children: proceedingText}, 
                    {type: "EXPRESSION", props: {}, children: `<noquote>()=>{return ${expression}}<noquote>`}
                ]
                , next]
        }
    }
    ...
    if(isClosingTag){
        ...
        if(tagStack[tagStack.length - 1] === type){
            ...
            if(typeof body == "string"){
                return [{type: "TEXT_ELEMENT", props: {}, children: body}, next]
            }else{
        ...
        if(isSelfClosingTag){
            ...
            if(isAComponent){
                return [{type, props, children: null, componentFunction: `<noquote>${type}<noquote>`}, next]
            }else{
                return [{type, props, children: null}, next]
            }
        }else{
            tagStack.push(type)
            let children = []
            ...
            while(next){
                ...
                if(currentScope === scope){
                    ...
                    if(isAComponent){
                        return [{type, props, children, componentFunction: `<noquote>${type}<noquote>`}, next]
                    }else{
                        return [{type, props, children}, next]
                    }
                }
            }
            if(isAComponent){
...
function transpile(source) {
    ...
    const converted = jsxToCreateElementMine4(jsx)
    ...
    // console.log(source.replace(jsxMatch[0], `${JSON.stringify(converted).replaceAll(/('|")<noquote>/g, "").replaceAll(/<noquote>('|")/g, "")}`))
    return source.replace(jsxMatch[0], `${JSON.stringify(converted).replaceAll(/('|")<noquote>/g, "").replaceAll(/<noquote>('|")/g, "").replaceAll(/\\\\("|')/g, "\'").replaceAll(/\\("|')/g, "\'").replaceAll(/\\("|')/g, "\'")}`)
}
